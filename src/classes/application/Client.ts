...
export class Client {
        ...
        bot.updates.on("message_new", async (context) => {
            if (typeof context.chatId === "number")
                return this.onChatMessage(
                    context as MessageContext & {chatId: number}
                );
            else
                return this.onPrivateMessage(
                    context as MessageContext & {isChat: false}
                );
        });
    ...
    private async executeCommand(
        ...
        event: ChatMessageEvent | PrivateMessageEvent
    ): Nothing {
        ...
        const dtoIndex = Reflect.getMetadata(
            "injectable-dto",
            executor,
            "execute"
        );
        ...
        for (let i = 0; i < paramTypes.length; i++) {
            if (i === dtoIndex) {
                ...
                const dto = await CommandDtoAdapter.getDto(
                    DtoClass,
                    event.args
                ).catch(() => null);
                if (dto === null) {
                ...
                return await executor
                    .execute(event, dto as Function)
                    .catch(async (err) => {
                        if (err instanceof BaseCommandException)
                            await event.replyLocalizedMessage(err.path);
                    });
            }
        }
    ...
    private async onButtonEvent(context: MessageEventContext): Nothing {
        const event: ButtonEvent = new ButtonEvent(context);
        const executor: ButtonExecutor | null = event.findExecutor();
        if (!executor) return;
        await executor.execute(event).catch(async (err) => {
            if (err instanceof BaseCommandException)
                await event.showLocalizedSnackbar(err.path);
        });
    }
}
