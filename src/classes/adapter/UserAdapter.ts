...
export class UserAdapter {
    ...
    public static getIdFromArgument(args: string[], arg: number = 0, allowGroups: boolean = false) : User | null {
        ...
        if ((match = currencyText.match(Regex.URL_GET))) {
            ...
            if (isNaN(+userId)) {
                ...
                return user.userId && (user.userId > 0 || allowGroups) ? new User(user.userId) : null;
            }
            userId = +userId;
            return (userId && (userId > 0 || allowGroups)) ? new User(userId) : null;
        }
