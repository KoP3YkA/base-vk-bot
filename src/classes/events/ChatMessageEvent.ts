...
export class ChatMessageEvent extends BaseClientEvent {
    ...
    public findExecutor() : ChatCommandExecutor | null {
        ...
        const command : Function | undefined = GlobalVariables.CHAT_COMMANDS.get(this.firstArgument);
        if (command) return new (command as new () => ChatCommandExecutor)();

        const alias : Function | undefined = GlobalVariables.CHAT_COMMAND_ALIASES.get(this.firstArgument);
