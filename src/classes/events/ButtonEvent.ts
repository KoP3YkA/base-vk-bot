...
export class ButtonEvent extends BaseClientEvent {
    ...
    public constructor(public readonly apiEvent: MessageEventContext) {
        super();
    ...
    public showLocalizedSnackbar = async (
        path: string,
        rename: Rename = {}
    ): Nothing =>
        await this.showSnackbar(
            Main.localization.getLocalizedText(path, rename)
        );

    public findExecutor(): ButtonExecutor | null {
        if (!this.command) return null;
        const command: Function | undefined = GlobalVariables.BUTTONS.get(
            this.command
        );
