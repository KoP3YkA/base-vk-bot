...
export class Main {
    ...
    public async start() : Nothing {
        ...
        Main.localization = new Localization(path.resolve(process.cwd(), "localization.yml"));
        Logger.info('Localization was cached')

        await Scanner.scanAndImportDirectory(path.resolve(__dirname, "../logic"))
        Logger.info('Logic directory was imported')

        const isDevMode : boolean = EnvironManager.getEnvironField<number>('MODE', (val: string) => +val) === 1
        ...
        await ModularORM.getInstance().start({
            ...
            port: EnvironManager.getEnvironField<number>('MYSQL_PORT', (val: string) => +val),
            logs: isDevMode,
        ...
        await new Client(EnvironManager.getEnvironField('BOT_TOKEN')).startPolling();

        const endTime : Date = new Date();
        Logger.info(`Bot has been started in ${endTime.getTime() - startTime.getTime()} ms`, 1);
    }
